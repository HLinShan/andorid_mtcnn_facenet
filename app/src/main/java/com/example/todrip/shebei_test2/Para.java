...
public class Para {
    ...
    {
        String time1_value = sp.getString("t0","300");//获取不到t1则默认为1
        t0.setText(time1_value);
        String time2_value = sp.getString("t1","5000");//获取不到t2则默认为5
        t1.setText(time2_value);
        String value = sp.getString("t2","0.76");//获取不到t3则默认为0.95
        //t3获取到的是sting类型，给他赋予的也是Sting类型
        t2.setText(value);
        //t3 下载info地址
        t3.setText(sp.getString("t3","http://youkangbao.cn/ykb/back/info/download.php?id=1"));
        //t4 下载feature地址
        t4.setText(sp.getString("t4","http://youkangbao.cn/ykb/back/face/download.php"));
        //t5 上传ID签到地址
        t5.setText(sp.getString("t5","http://youkangbao.cn/ykb/back/checkin/add.php"));//获取不到t4则默认为www.aaa.com
    }
}
